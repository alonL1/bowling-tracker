...
export async function GET(request: Request) {
  try {
    ...
    let query = supabase
      .from("games")
      .select(
        "id,game_name,player_name,total_score,status,played_at,created_at,session_id,scoreboard_extraction,selected_self_player_key,selected_self_player_name,tags,session:bowling_sessions(id,name,description,started_at,created_at)",
        {
          count: "exact"
        }
      )
