...
function getGamesSelect(metric: LeaderboardMetric | null): string {
  ...
  if (metric && !["SpareRate", "TotalSpares", "MostNines"].includes(metric)) {
    return "id,user_id,total_score,session_id,played_at,created_at,tags,frames:frames(is_strike,is_spare)";
  }

  return "id,user_id,total_score,session_id,played_at,created_at,tags,frames:frames(is_strike,is_spare,shots:shots(shot_number,pins))";
}
